...
def _model_to_detail(transaction: models.Transaction) -> api_schemas.TransactionDetailResponse:
...
class TransactionService:
    def __init__(
        self,
        uow: UnitOfWork,
        kafka: KafkaProducerWrapper | None = None,
    ):
        self.uow = uow
        self.kafka = kafka

    async def _send(self, topic: str, payload, key: UUID | str) -> None:
        if not self.kafka:
            return
        ...
        ok = await self.kafka.send_json(topic, data, key=str(key))
        if not ok:
            logger.warning("Kafka event was not delivered: topic=%s key=%s", topic, key)
    ...
    async def create_manual_transaction(
        ...
        await self._publish_created_events(transaction)
    ...
    async def import_mock_transactions(
        ...
        for item in items:
            if request_user_id:
                if item.user_id and item.user_id != EMPTY_UUID and item.user_id != request_user_id:
            ...
            async with self.uow:
                if await self.uow.transactions.exists(transaction.transaction_id):
                    continue
                self.uow.transactions.create(transaction)
                await self.uow.flush()

            await self._publish_imported_events(transaction)
            count += 1
    ...
    async def delete_transaction(self, transaction_id: UUID) -> None:
        async with self.uow:
            transaction = await self.uow.transactions.delete_by_transaction_id(transaction_id)
            if not transaction:
                return
            await self.uow.flush()
            user_id = transaction.user_id
    ...
    async def _publish_created_events(self, transaction: models.Transaction) -> None:
        transaction_type = enums.type_from_db(transaction.type)
        await self._send(
        ...
        if (
            ...
            await self._send_goal_event(transaction)

    async def _publish_imported_events(self, transaction: models.Transaction) -> None:
        await self._send(
    ...
    async def _send_goal_event(self, transaction: models.Transaction) -> None:
