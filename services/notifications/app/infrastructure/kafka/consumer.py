...
async def consume_loop(
    ...
    logger.info("Kafka consumer started (listening to %s and %s)", 
                settings.KAFKA.KAFKA_TOPIC_EVENTS, settings.KAFKA.KAFKA_TOPIC_AUTH)
        ...
        while True:
            ...
            for tp, messages in result.items():
                ...
                if highwater is not None:
                    ...
                    metrics.KAFKA_CONSUMER_LAG.labels(topic=tp.topic, partition=tp.partition).set(highwater - current_offset)

                await process_batch(messages, tp.topic, db_session_maker, arq_pool, dlq_producer)
