...
async def run_outbox_loop(ctx) -> None:
    ...
    while True:
        try:
            ...
            logger.error("Budgets outbox loop error: %s", exc, exc_info=True)
...
async def renew_monthly_budgets_task(ctx) -> int:
    ...
    try:
        ...
        if created_count:
            logger.info(
                "Monthly budgets renewed",
                extra={"created_count": created_count},
            )
        return created_count
    except Exception as exc:
        logger.error("Monthly budget renewal failed: %s", exc, exc_info=True)
        raise
